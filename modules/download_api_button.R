...
download_api_button_server <- function(id,
    ...
    moduleServer(id, function(input, output, session) {
         ...
         observeEvent(input$download_api_button, {
               ...
               tstart <- as_datetime(ifelse(is.null(daterange$selected_start_date()),
                                            NA, daterange$selected_start_date()))
               tend <- as_datetime(ifelse(is.null(daterange$selected_end_date()),
                                          NA, daterange$selected_end_date()))
               time_start <- daterange$selected_start_date() %>% as.POSIXct()
               time_end <- daterange$selected_end_date() %>%  as.POSIXct()
               log_trace("mod download: Download pushed with paremeters Type:
               ...
               if(any(is.na(c(type, name, tstart, tend)))) {
                   ...
                   shinyalert("Be patient!", "Data will be gathered, this can take
                              some time. Please check the other button if data is
                              available for usage.", type = "info")
