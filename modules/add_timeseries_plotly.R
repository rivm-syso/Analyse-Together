...
timeseries_server <- function(id, data_measurements_stations, overview_component) {
  ...
  moduleServer(id, function(input, output, session) {
                 ...
                 output$timeseries_plot <- renderPlotly({
                   ...
                   # Make a plot
                   shiny::validate(need(!is_empty(data_timeseries$value),"Geen sensordata beschikbaar"))

                   p_plot <- ggplot(data = data_timeseries, aes(x = date, y = value, group = station)) +
                     ...
                     labs(x = "Date", y = paste("Concentration (", "\U03BC", "g/","XX",")"), title=paste0('Timeseries for: ', parameter_label)) +
                     expand_limits(y=0) +
                     theme_bw() +
